    ...
    <div class="parking-list-container" id="main-body">
            ...
            <div class="filter">
                ...
                if (isset($_GET["sortBy"])) {
                    ...
                    <select name="sort-by" id="sort-by" class="dropdown" onchange="doReload({sortBy:this.value})">
                        ...
                        <option value="price" <?php echo ($_GET["sortBy"] == "priceAsc" || $_GET["sortBy"] == "priceDesc") ? "selected" : "" ?>>Pay-Per-Hour</option>
        ...
        <div class="parking-list-explorer-container">
            ...
            if (isset($_GET["sortBy"])) {
                try {
                    if ($_GET["sortBy"]) {
                        usort($result, "sortBy" . ucfirst($_GET["sortBy"]));
                    }
                } catch (\Throwable $th) {
                    //throw $th;
                }
            ...
            foreach ($result as $row) {
                if (isset($_GET["filter"])) {
                    if (!str_contains(strtolower($row["address"]), strtolower($_GET["filter"]))) {
                        continue;
                    }
                }
...
<script>
    document.getElementsByClassName("search-icon")[0].addEventListener("click", (e) => { doReload({ "filter": e.view.document.getElementById("search").value }) })
    document.getElementById("search").addEventListener("keyup", (e) => {
        if (e.code == "Enter") {
            doReload({ "filter": e.target.value })
        }
    })
